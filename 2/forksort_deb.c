 ...
 * 
 *  Kill all processes: pgrep -u "$(whoami)" forksort | xargs kill
 * 
 */
...
int close_pipe_ends(child_proc_t *child, char mode){
...
int main(int argc, char *argv[])
{
    ...
    while((next_line_length = getline(&next_line, &next_line_length, stdin)) != EOF){
        ...
        // enforce_newline(&next_line, next_line_length);
    ...
    {
        if(DEBUG > 0) printf("%s", first_line);
        free(first_line);
        exit(EXIT_SUCCESS);
    }

    /* close write pipe to signalize finished reading */
    if(close_pipe_ends(sort_left, 'w') == -1 || close_pipe_ends(sort_right, 'w') == -1){
        // exit?
    }
    ...
    if(close_pipe_ends(sort_left, 'r') == -1 || close_pipe_ends(sort_right, 'r') == -1){
        // exit?
    }
